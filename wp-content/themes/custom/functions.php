...
function starkers_setup() {
	...
	add_image_size( 'leadership-team-images', 350, 363, true );
	add_image_size( 'home-slider-images', 1366, 768, true );
	add_image_size( 'home-find-out-more-images', 410, 300, true );
	add_image_size( 'news-thumb-image', 350, 234, true );
	add_image_size( 'projects-thumb-image', 330, 221, true );
...
function starkers_widgets_init() {
	...
	register_sidebar( array(
		'name' => __( 'Copyright Widget Area', 'starkers' ),
		'id' => 'copyright-widget-area',
		'description' => __( 'The copyright widget area', 'starkers' ),
		'before_widget' => '<div>',
		'after_widget' => '</div>',
		'before_title' => '<h3>',
		'after_title' => '</h3>',
...
/*-----------------------Add Custom Logo Upload in wp customizer-------------------*/
...
// Shortcode to output logo in Footer
function footer_logo( $atts ) { 
	
	$custom_logo_id = get_theme_mod( 'custom_logo2' );
	//echo $custom_logo_id;
	$logo = wp_get_attachment_image_src( $custom_logo_id , 'full' );
	if ( has_custom_logo() ) {
			echo '<a href="'.site_url().'"><img src="'. $custom_logo_id .'" width="140" style="margin-top: 10px;" alt="amp-logo-footer"></a>';
	} else {
			echo '<h1>'. get_bloginfo( 'name' ) .'</h1>';
	}
	}
add_shortcode( 'my_vc_locations_output', 'footer_logo'); 